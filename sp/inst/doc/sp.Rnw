...
of class or extending {\tt SpatialPoints}, we strongly encourage users to
access the data by using functions and methods, e.g. {\tt coordinatas(x)}
to retrieve the coordinates.

Selecting, retrieving or replacing certain attributes in spatial objects
with attributes can be done by the methods
\begin{itemize}
