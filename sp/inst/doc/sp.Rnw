...
grd = SpatialGrid(points = SpatialPoints(pts))
summary(grd)
@
Note that when passed a points argument, SpatialGrid also accepts a
tolerance (default 10 * .Machine\$double.eps) to specify how exactly the
points should be on a grid. For very large coordinates, this value may
...
<<>>=
xy = expand.grid(xc = 1:3, yc = 1:3)
xy.grd = data.frame(xy, z = 1:9)
coordinates(xy.grd) = ~xc + yc
gridded(xy.grd)
gridded(xy.grd) = TRUE
gridded(xy.grd)
summary(xy.grd)
...
\subsection{Storage mode: as points or as matrix/array?}
<<>>=
fullgrid(xy.grd)
@
