...
specifying {\em how} polygons intersect\footnote{of sp versions
larger than 1.2-0, and rgeos versions larger than 0.3-12}:
<<>>=
p$z = 1:9 # add cell number as attribute values
class(p)
as.data.frame(aggregate(p, p, sum))[[1]]
as.data.frame(aggregate(p, p, sum, minDimension=-1))[[1]]
as.data.frame(aggregate(p, p, sum, minDimension= 0))[[1]]
as.data.frame(aggregate(p, p, sum, minDimension= 1))[[1]]
as.data.frame(aggregate(p, p, sum, minDimension= 2))[[1]]
as.data.frame(aggregate(p, p, sum, areaWeighted = TRUE))[[1]]
@
...
\code{over} to constrain the intersecting polygons.
