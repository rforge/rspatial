...
setClass("SpatialData", 
	...
	prototype = list(bbox = matrix(rep(NA, 6), 3, 2),
		proj4string = CoRS(as.character(NA))),
	validity = function(object) {
		...
		if (!is(object@proj4string, "CoRS"))
			return("proj4string should be CoRS")
...
# if needed add them
		...
		if (missing.i && missing.j) {
			...
			i = TRUE
		if (is.matrix(i))
			stop("matrix argument not supported in selecting from SpatialDataFrame")
...
proj4string = function(sd) { 
	...
	CoRSargs(sd@proj4string)
}
"proj4string<-" = function(sd, value) { 
	if (!is(sd, "SpatialData"))
		stop("proj4string only works for classes inheriting from SpatialData")
	if (!is(value, "CoRS"))
		stop("assigned value must be CoRS object")
...
summary.SpatialData = function(object, ...) {
	...
	obj[["proj4string"]] = CoRSargs(object@proj4string)
	...
	if (is(object, "SpatialDataPolygons")) {
		obj[["n.polygons"]] = length(object@polygons)
...
"polygons<-" = function(obj, value) {
	if (!is.null(obj) && !is.data.frame(obj))
		stop("obj is not a data.frame")
	if (any(x <- !as.logical(sapply(value, verifyPolygon))))
		stop(paste("polygon(s)", paste(which(x), collapse = " "), "not valid"))
	rangePolygons = function(x) as.vector(apply(x, 2, range))
	r = t(sapply(value, rangePolygons))
	bbox = matrix(NA, 2, 2)
	bbox[1,1] = min(r[,1])
	bbox[1,2] = max(r[,2])
	bbox[2,1] = min(r[,3])
