...
lines     & {\tt SLines}                 & No          &{\tt Spatial}*, {\tt Line} list \\
lines     & {\tt SpatialLines}           & No          &{\tt Spatial}*, {\tt SLines} list \\
lines     & {\tt SpatialLinesDataFrame}  & Yes         &{\tt SpatialLines}* \\
rings     & {\tt Polygon}                  & No          &{\tt SLine}* \\
rings     & {\tt Polygons}                 & No          &{\tt Spatial}*, {\tt Polygon} list \\
rings     & {\tt SpatialPolygons}           & No          &{\tt Spatial}*, {\tt Polygons} list \\
rings     & {\tt SpatialPolygonsDataFrame}  & Yes         &{\tt SpatialPolygons}* \\
\end{tabular}
\end{center}
...
plot.SpatialPolygons(SpP, col = 1:3, pbg="white")
# plot(SpP)
@
