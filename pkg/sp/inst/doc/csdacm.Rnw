...
\title{ Customising spatial data classes and methods\footnote{This vignette formed pp. 127--148 of the first edition of Bivand, R. S.,
...
traditional \SIII system and the more recent \SIV system (see Section
\ref{classes:here}, in which classes were described for the use{\RR} ---
...
add \code{plot} and \code{spsample} methods for them. Method \code{overlay}
...
\code{overlay} methods for these classes:
\begin{footnotesize}
<< >>= 
