...
pixels    & {\tt SpatialPixelsDataFrame} & Yes         &{\tt SpatialPixels}* \\
          &                              &             &{\tt SpatialPointsDataFrame}** \\
full grid & {\tt SpatialGrid  }          & No          &{\tt SpatialPixels}* \\
full grid & {\tt SpatialGridDataFrame}   & Yes         &{\tt SpatialGrid}* \\
...
it is meant to represent a hole in the {\tt Sr3} polygon. The default value for the hole colour {\tt pbg} is {\tt "transparent}, which will not show, but which often does not matter, because another polygon fills the hole --- here it is set to {\tt "white"}. Note that the {\tt Srings} objects have to be given character ID values, and that these values must be unique for {\tt Srings} objects combined in a {\tt SpatialRings} object.
<<fig=TRUE>>=
...
(data.frame). The {\tt row.names} of the attributes data frame are matched with the ID slots of the {\tt SpatialRings} object, and the rows of the data frame will be re-ordered if necessary.
<<>>=
