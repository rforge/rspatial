...
\begin{itemize}
\item[\[] select rows (points) or columns (variables)
\item[\[\[] retrieve a column from the attribute table (data.frame part)
\item[\[\[<-] assign or replace a column in the attribute table (data.frame part)
