...
style (Chambers, 1998) to allow validation of objects created. Although
we mainly aim at using spatial data in the geographical (two-dimensional)
domain, the data structures that have a straightforward implementation
in higher dimensions (points, grids) do allow this.
...
With this package we hope that other maintainers of packages dealing
with spatial data will either convert their package to supporting the
spatial data classes provided here, or make their package compatible
with these classes. Even if only conversion routines to and from {\tt sp}
classes become available it would be useful, as one-to-many conversions
are much easier to deal with than many-to-many.
...
{\tt spplot}, a front-end to {\tt levelplot}, this form is required; in
